...
test('Tamil reverse Gate 0 freezes the structured success result for canonical output', () => {
  expect(reverseTamilInput('கீ³தா', { inputMode: 'tamil-precision', outputMode: 'canonical' })).toEqual({
    status: 'success',
    inputKind: 'tamil-precision',
    canonicalRoman: 'gItA',
  });
...
test('Tamil reverse Gate 0 keeps rejection identifiers stable across phase-1 classes', () => {
  expect(reverseTamilInput('கு³ரு ரு', { inputMode: 'tamil-precision', outputMode: 'canonical' })).toMatchObject({
    status: 'rejected',
...
test('Tamil reverse Gate 0 does not let display outputScheme select parser mode implicitly', () => {
  expect(detransliterate('கீ³தா', { outputScheme: 'sanskrit-tamil-precision' })).toBe('கீ³தா');
  expect(reverseTamilCanonical('கீ³தா')).toBe('gItA');
});
...
test('Gate 3 rejects incomplete or ambiguous precision marker fragments instead of guessing', () => {
  ...
  expect(reverseTamilRejection('கு³ரு ரு')).toMatchObject({ status: 'rejected', inputKind: 'mixed-ambiguous' });
  expect(reverseTamilRejection('கு^3ரு ரு<R')).toMatchObject({ status: 'rejected', inputKind: 'mixed-ambiguous' });
});
...
test('Gate 5 keeps paste canonicalization canonical while Tamil precision remains output-only', () => {
  ...
  expect(canonicalizeDevanagariPaste('ॐ')).toBe('OM');
  expect(reverseTamilCanonical('கீ³தா')).toBe('gItA');
  expect(canonicalizeAcceptedInputToken(reverseTamilCanonical('கீ³தா'))).toBe('gItA');
});
